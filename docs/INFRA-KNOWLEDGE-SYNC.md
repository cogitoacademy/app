...
|---|---|---|
| **GitHub Actions secrets** | `COOLIFY_PROD_SERVER_WEBHOOK`, `COOLIFY_PROD_WEBHOOK`, `PROD_DATABASE_URL`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET` | Only the CD pipeline (deliberate exception — CI can't read your vault) |
...
So the vault is not *only* "Coolify app secrets" — it's "everything the
...
|---|---|---|---|
| **Postgres 16 + Redis 7** | **No — they already exist** as running Coolify containers. The plan explicitly says: *"keep the existing running containers, bring them under Ansible-declared Coolify config... **Never recreate (data)**."* | Yes — Ansible re-declares them (names, volumes, private network) so drift-check can verify the UI matches the config | Yes — `DATABASE_URL` / `REDIS_URL` |
| **R2 buckets** | **Yes** — Terraform creates `cogito-infra-state` + `cogito-backups` | Yes | Yes — R2 creds |
| **DNS records** | **Yes** — Terraform owns `api.`/`app.`/`status.`/`coolify.` | Yes | n/a |
| **Xendit, Resend, Google, Sanity** | **No — external SaaS**, nothing to build | No — their *outputs* (keys, webhook URLs, redirect URIs) become vault/GitHub values | Yes — that's the whole wiring |
...
*re-declare* them so the state is reproducible and drift-checkable — not to
...
*"the code is designed to fail loudly, retry safely, and leave audit trails
...
| Gap | Reality |
|---|---|
| **In-memory fallbacks** | If Redis fails at runtime, idempotency/rate-limit/circuit-breaker fall back to per-process in-memory stores — they degrade cross-instance guarantees (documented in CONTEXT.md). Not "impossible to have stale state" — a *degraded mode*. |
...
**Public IP:** `15.235.186.159` (OVH). This is the *only* public address. All
...
- **Outbound**: containers share the host's public IP via NAT. **This is why
  ...
  checks the *client* IP; and when the server calls Xendit, Xendit sees the
...
reachable *only* from `100.64.0.0/10` — invisible to the public internet.
...
2. **R2 creation as an explicit Phase-0 prerequisite** — it's a hard boot
   blocker (env guard) *and* the backup target, but the plan lists it only
...
4. **Drizzle Studio via tailnet tunnel** — document the safe path (SSH tunnel
   → local `bun run db:studio`), explicitly *not* on the prod container.
...
8. **Backup `DATABASE_URL` host-reachability note** — the vault's
   `DATABASE_URL` must resolve from the VPS *host* (published port or
