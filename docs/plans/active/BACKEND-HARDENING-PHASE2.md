    ...
    try {
      ...
      const res = await emailPort.send({
        ...
        category: notif.category as
          "booking" | "payment" | "refund" | "schedule" | "override",
      });
...
Mirror the same call with `category: NOTIFICATION_CATEGORY.REFUND` in the admin refund path (`eventKey: payment.{id}.refunded`). 2. In `payment/index.ts`, accept `notificationPort?: NotificationPort` in `createPaymentModule` deps and pass it to the service. 3. In `services.ts`, wire `notificationPort: notification.service`. 4. Verify `EMAIL_SUPPORTED_CATEGORIES` in `notification.service.ts` already includes `payment`/`refund`; if not, add them so the dispatch row is queued.
...
if (isPastCancellationDeadline(session)) {
  const participant = await repo.findSessionParticipant(
    tx,
    session.bookingId,
    userId,
  );
