...
| Live env gaps        | **Historical snapshot (pre-apply).** **All wiring confirmed complete by the operator 2026-09-01** — payments (Midtrans Sandbox + UAT emails), Google Meet, R2, Sanity, backups, and `status.` DNS are live; 47 env vars were applied from the SOPS vault on 2026-08-31.                                                                                                                                                                                                                                                                                                         |
...
4. **Midtrans Sandbox wiring**: set `PAYMENT_PROVIDER=midtrans` + `MIDTRANS_MODE=test` + Sandbox `MIDTRANS_SERVER_KEY`/`MIDTRANS_CLIENT_KEY`/`MIDTRANS_MERCHANT_ID` + `PAYMENT_TEST_ALLOWED_EMAILS` in the vault. Configure the Midtrans dashboard Payment Notification URL as `https://api.cogitoacademy.id/webhooks/payments/midtrans`. `WEBHOOK_ALLOWED_IPS` stays optional; body `signature_key` verification is the primary gate.
...
- [x] `infra/secrets/prod.env` (encrypted) with ALL credentials the user has: `BETTER_AUTH_SECRET`, `PAYMENT_WEBHOOK_SECRET`, `ADMIN_EMAILS` (default `itcogitoacademy01@gmail.com` — confirm), `RESEND_API_KEY`+`EMAIL_FROM`, `PAYMENT_PROVIDER=midtrans`, `MIDTRANS_MODE` + matching Sandbox/Production `MIDTRANS_SERVER_KEY`/`MIDTRANS_CLIENT_KEY`/`MIDTRANS_MERCHANT_ID`/`MIDTRANS_WEBHOOK_SIGNATURE_KEY`, `PAYMENT_TEST_ALLOWED_EMAILS`, `GOOGLE_CLIENT_ID/SECRET`, `GOOGLE_MEET_*` (+ refresh token via `scripts/google-meet-auth.ts`), `R2_*`+`R2_PUBLIC_URL`+`R2_BACKUP_BUCKET`, `SANITY_*`, `METRICS_TOKEN`, `DATABASE_URL`/`REDIS_URL` (existing containers). **DONE 2026-08-28** — encrypted vault committed; Midtrans cutover is documented in `docs/MIDTRANS-MIGRATION.md`.
...
- [x] Repo fix: configure Google sign-in with `prompt=consent` so the OAuth permission screen is visible in verification evidence; keep the broader Calendar scope isolated to the dedicated Meet OAuth flow (2026-09-01).
...
- [x] Midtrans: deploy with `PAYMENT_PROVIDER=midtrans` + `MIDTRANS_MODE=test` + matching Sandbox credentials + UAT/IP allowlists first; **production-domain sandbox E2E, then switch to Live Mode for one real small transaction** (RUNBOOK checklist). **Sandbox wiring complete; remaining go-live step: Midtrans Live Mode E2E (one real small transaction).**
...
- Phase 2: `/health` + sha ok with full env; production-domain provider Test Mode UAT → Live Mode E2E (Midtrans); Meet probe ok; R2 round-trip ok. **Env wiring confirmed complete by the operator (2026-09-01: payments/Meet/R2/Sanity all live; `/health` sha-verified). Live Mode E2E (real transaction) remains the payment-provider go-live step.**
...
- **Payment-provider go-live (Midtrans)**: production can stay on Sandbox during UAT, restricted by `PAYMENT_TEST_ALLOWED_EMAILS`; switch to Live Mode only after Sandbox E2E and then run one real small transaction.
