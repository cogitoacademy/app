...
| D5  | PR #55 (`f/frontend-prd-gaps`) blockers were reported: stale type-check diagnostics, achievement migration/schema mismatch, missing F18/J2/dead-component audit sections, and stray QA artifacts. The branch now resolves these through `tsgo`, an end-to-end `awardingDate`/`evidenceUrl`/`documentationUrl` contract, audited spec reconciliation, and artifact removal. | PR #55                                       | Resolved in PR #55, pending CI |
...
- [ ] **Step 1:** Post a precise PR #55 review comment with each blocker + suggested fix:
  ...
  - [x] Stray `.qa-marks-before/` and `artifacts/` (~2.7 MB) removed from the branch and ignored going forward.
