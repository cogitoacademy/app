...
| G12 | Google Meet attendee automation           | FR-21, OQ-05               | Medium   | 2d     | meeting       | PARTIAL (no update-on-reschedule / delete-on-cancel — MeetingPort only has createEvent)             |
| G13 | Offline room availability                 | FR-22                      | Low      | 1d     | room          | PARTIAL (checkAvailability exists but NOT integrated into booking creation)                         |
| G14 | Admin room approval                       | FR-22                      | Low      | 1d     | room          | PARTIAL (assignRoom doesn't transition to SCHEDULED; no student notifications; expire auto-cancels) |
| G15 | Group series no opt-out disclaimer        | FR-20                      | Low      | 0.5d   | booking       | PARTIAL (UNREACHABLE — createSeries hardcodes targetGroupSize:1, no group-series creation)          |
...
Audit of git HEAD `9b7df5e` against the PRD (FR/DL/TC refs). These are newly discovered gaps found _after_ G1–G20 merged. They are **folded into `docs/plans/active/BACKEND-HARDENING-PHASE2.md` PR 5**, not this spec's G-series.
...
| --- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- |
| B3  | High     | Group booking with 2 ≤ headcount < target EXPIRES at the 12h deadline instead of repricing + reconfirming (FR-16/TC-18 violation)        | `expireBookings` `booking.service.ts:2009-2048` has no headcount branch |
| B4  | Medium   | Knowledge Bank eligibility uses `availableBalance` not total balance (DL-16 violation)                                                   | `wallet.service.ts:431`                                                 |
| B6  | Medium   | No payment/refund notifications at all (notification matrix rows unfulfilled)                                                            | `payment.service.ts` writes none                                        |
| B8  | Medium   | Group-series creation flow missing entirely — `createSeries` hardcodes `targetGroupSize:1` (FR-20 TC-24/25/27/28/30/32-34 unimplemented) | `booking.service.ts:1881`                                               |
| B9  | Low-Med  | `cancelSession` after H-2 throws instead of forfeiting Marks (series rules)                                                              | `booking.service.ts:1134-1140`                                          |
...
| OQ-04                                           | Admin SLA escalation via WhatsApp (+62 881-0119-90195) — 30 min business hours, 4 hours outside                                                                                                                                   | **Still not implemented (2026-08-14)** — G1 support ticket creates the queue but no WhatsApp escalation exists       | Add SLA timer + WhatsApp escalation to support ticket flow (tracked in BACKEND-HARDENING-PHASE2.md) |
