...
- Added transactional fan-out of a durable `Cogito rate updated` in-app notification to every current tutor for each changed economy version; identical saves are no-ops.
...
- Integration: packages/api/src/tests/integration/economy-roles.test.ts — 5 passed, including tutor notification fan-out and no-op protection.
- Browser E2E: packages/e2e/src/specs/economy-roles.spec.ts — 3 passed (student, tutor, admin; admin update is verified in the tutor notification center).
