...
2. **S7 webhook guard (Option A)** — in `cd-prod.yml`, before the curl: if `${{ secrets.COOLIFY_PROD_SERVER_WEBHOOK }}` is empty, `echo` a clear message ("COOLIFY_PROD_SERVER_WEBHOOK is unset — configure the Coolify resource webhook and add it as a GitHub secret") and `exit 1`. Keep `--fail --max-time 30`. **Root cause is documented in DEPLOYMENT-PLAN.md Task 0.2**: the webhook URL host (`coolify.cogitoacademy.id`) has no DNS record because the control plane is tailnet-only; the operator recreates the secrets with the resolvable `https://coolify.cogitoacademy.id/api/v1/deploy?uuid=...` URL (only the deploy-webhook path is exposed; the UI stays tailnet-only). Commit: `fix(ci): fail loudly when Coolify deploy webhook secret is unset`
