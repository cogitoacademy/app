...
- v1.7 (2026-08-19): **Wave-6 plan COMPLETE** — all H/M/L/N + P findings fixed & merged to main via PR #82 (`fix/wave6-a`), PR #83 (`fix/wave6-b`), PR #84 (`fix/wave6-c`). L3 closed as defense-in-depth (the app-level Redis-backed `authRateLimit` covers `/api/auth/email-otp/*`; better-auth's internal per-plugin 3/min is the only remaining gap, deferred pending an `auth`↔`api` circular-dependency refactor to wire Redis `secondaryStorage`).
- v1.8 (2026-09-02): **P2/G2 email delivery consolidated** — the automatic email/password signup OTP now carries the welcome/onboarding copy and login link in the same `auth` email. The separate `WelcomeEmailSender`, signup database-hook delivery, and `buildWelcomeEmail` template were removed; non-signup verification/resend and other OTP types remain purpose-specific. Added regression coverage for the signup context flag and combined template; no API or schema contract changed.
