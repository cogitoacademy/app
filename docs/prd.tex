...
\begin{itemize}
	...
	\item tutor self-pricing with Cogito floor prices,
...
\hline
	\textbf{Document Status} & Client Review \\
\hline
...
\begin{tabular}{|p{3cm}|p{3cm}|p{8.2cm}|}
	...
	\hline
	1.1              & June 3, 2026  & Clarified tutor self-pricing economics: Cogito tables define baseline floors, tutors may price above floor, and Cogito earns 1 extra Mark for every 5 accumulated Marks above baseline. \\
	\hline
...
\hline
DL-15 & May 30, 2026 & Tutor tier-based pricing is removed from Phase 0; standard session prices are tutor-set per group size and must meet Cogito floor prices. Any tier display, if retained later, is only a credibility signal and does not affect pricing. & Final \\
\hline
...
\hline
DL-22 & June 3, 2026 & The online/offline pricing tables are baseline floor schedules. Tutors may set prices above the baseline. For every 5 Marks of accumulated price increase above the baseline total, Cogito receives 1 additional Mark; the remaining increase is added to tutor share. & Final \\
\hline
...
\begin{itemize}
    \item \textbf{OQ-01: Resolved.} Client confirmed Explorer Pack at 200 Marks, Rp 1,570,000 (~Rp 7,800 / Mark base + payment gateway processing fee). Earlier draft value of Rp 3,320,000 is superseded.
    ...
    \item \textbf{OQ-06: Resolved.} Purchased Marks cannot be converted back to rupiah. Unused Marks remain in the student's wallet because Marks do not expire. Cash refunds are limited to actual payment errors or admin-approved corrections where cash was captured incorrectly.
...
\begin{itemize}
	...
	\item tutor self-pricing and floor validation,
	\item tutor list, summary profile, credentials summary, expertise, price, and availability visibility,
...
\begin{itemize}
	...
	\item automated tutor payout accounting inside Phase 0 unless separately approved,
...
\subsection{Tutor}
The tutor is an invite-only supply-side user. A public user cannot self-select the tutor role during signup. A tutor receives incoming bookings, manages availability, accepts or declines sessions, sets session prices at or above Cogito floors, and marks sessions complete after the tutor profile is approved and published.

\subsection{Admin}
The admin monitors bookings, wallet activity, tutor pricing compliance, and exceptions. The admin also performs corrections when needed.
...
\hline
Tutor & Invite acceptance, account claim, onboarding, dashboard, availability management, incoming bookings, booking detail, reschedule proposal, session completion, and session notes. \\
\hline
Admin & Login, dashboard, user lookup, tutor invitation, tutor onboarding review, booking monitor, booking detail and state history, wallet / ledger view, override / reconciliation, tutor price floor review, achievement moderation, and audit review. \\
\hline
\end{tabular}
...
\hline
Set own tutor prices & No & Yes & Yes & Tutors set prices for group sizes 1--6 at or above Cogito floors; admin can review or correct exceptions. \\
\hline
...
\begin{longtable}{|L{1.5cm}|L{4.1cm}|L{9.0cm}|}
	...
	\hline
	FR-04           & As a student, I want to buy Marks through a payment provider so I can fund bookings.                                   & \textbf{Requirement:} The system shall support Marks purchase through an external payment integration and approved package pricing. \newline \textbf{Acceptance:} Successful payment increases balance and records a receipt; failed or pending payments do not credit Marks. The package cards show Starter Pack (50 Marks / Rp 430,000), Learner Pack (120 Marks / Rp 990,000), Explorer Pack (200 Marks / Rp 1,570,000), and Pioneer Pack (300 Marks / Rp 2,180,000).                                                                                                                                                                                                                                                                                                                                                                 \\
	\hline
	FR-05           & As an admin, I want tutor prices to respect Cogito floors.                                         & \textbf{Requirement:} The system shall validate tutor-set prices against Cogito floor prices for each group size. \newline \textbf{Acceptance:} Admin can review tutor prices, prices below the floor cannot be published, no tier-based pricing logic exists in Phase 0, and prices above floor use the approved extra-take rule: Cogito receives 1 extra Mark for every accumulated 5 Marks above the baseline total.                                                                                                                                                                                                                                                                                                                                 \\
	\hline
	FR-06           & As a student, I want enough tutor information to make a booking decision.                                              & \textbf{Requirement:} The system shall provide a tutor list with summary profile, credentials summary, expertise, tutor-set prices in Marks, and availability. \newline \textbf{Acceptance:} The listing is usable without requiring a separate content system for these core booking details. Tutor cards and profiles must show the tutor's prices for supported group sizes, and every confirmed booking stores a price snapshot so future tutor price changes do not affect existing bookings.                                                                                                                                                                                        \\
	\hline
...
FR-12           & As a student, I want clear Knowledge Bank access rules so I know when content is available.                                           & \textbf{Requirement:} The system shall link to the existing Knowledge Bank and gate student access by login plus wallet threshold. \newline \textbf{Acceptance:} Knowledge Bank access is available only after signing into the student dashboard and only if the student's wallet has at least 35 Marks. The UI must state clearly that 35 Marks is a minimum wallet-balance requirement, not a 35-Mark access fee, and opening the Knowledge Bank must not deduct Marks.                                                                                                                                                                                                                                                                                                                                                                 \\
	...
	\hline
	FR-19           & As a tutor, I want to set my own prices so I can price my time appropriately.                         & \textbf{Requirement:} The system shall let each tutor set six standard session prices: one for each group size from 1 to 6 students. \newline \textbf{Acceptance:} Each price is stored in Marks, must be at or above the Cogito floor for that group size, has no ceiling, and changes do not retroactively affect confirmed bookings. When the tutor-set price is above the floor, the extra Marks are split using the accumulated extra-take rule: Cogito receives 1 extra Mark for every full 5 Marks above the baseline total, and the remainder is added to tutor share.                                                                                                                                                                                        \\
	\hline
...
\begin{itemize}
    ...
    \item \textbf{TutorProfile}: tutor bio, expertise, display prices, availability summary, onboarding status, publication status, and admin review metadata.
...
\begin{itemize}
    ...
    \item standard session prices for class sizes 1--6, validated against Cogito floors,
...
\begin{itemize}
	...
	\item original Marks amount, repriced Marks amount, hold amount, refunded amount.
...
\begin{itemize}
    ...
    \item All four package values are approved for Phase 0 seed data: Starter 50 Marks / Rp 430,000, Learner 120 Marks / Rp 990,000, Explorer 200 Marks / Rp 1,570,000, Pioneer 300 Marks / Rp 2,180,000.
\end{itemize}

\textbf{Approved package table}

The following package table is the approved Phase 0 wallet top-up structure. All four package prices are client-confirmed; the small gap between per-Mark rate and displayed IDR price is the payment gateway processing fee.

\begin{tabular}{|p{3.0cm}|p{3.0cm}|p{3.4cm}|p{3.4cm}|}
\hline
\textbf{Package} & \textbf{Marks} & \textbf{IDR Price} & \textbf{Effective Rate} \\
\hline
Starter Pack & 50 Marks & Rp 430,000 & ~Rp 8,500 / Mark \\
\hline
Learner Pack & 120 Marks & Rp 990,000 & ~Rp 8,200 / Mark \\
\hline
Explorer Pack & 200 Marks & Rp 1,570,000 & ~Rp 7,800 / Mark \\
\hline
Pioneer Pack & 300 Marks & Rp 2,180,000 & ~Rp 7,250 / Mark \\
\hline
\end{tabular}

The Explorer Pack price is Rp 1,570,000 (200 Marks at ~Rp 7,800 / Mark base + payment gateway processing fee), confirmed by the client. An earlier draft value of Rp 3,320,000 is superseded and is not used in Phase 0 seed data.

\textbf{Approved economics constants}
\begin{itemize}
    \item Student purchase rates vary by package size: smaller packages cost more per Mark and larger packages cost less per Mark.
    \item Approved student purchase rate range follows the approved package table: approximately Rp 8,500 / Mark (Starter Pack) to Rp 7,250 / Mark (Pioneer Pack). Per-Mark rates are the pre-fee base; displayed IDR prices include the payment gateway processing fee so the amount the student pays is the value shown in the table.
    \item Reference student purchase rate for illustrations: Rp 7,250 / Mark (Pioneer Pack), with the working-average rate of approximately Rp 7,800 / Mark available for Phase 0 illustrations.
    \item Tutor payout rate: Rp 7,000 / Mark.
    \item Reference exchange margin: Rp 250 / Mark at the Rp 7,250 reference average. This margin is separate from the session take.
\end{itemize}

\textbf{Ledger rules}
\begin{itemize}
	\item The wallet keeps \textbf{total balance} and \textbf{held balance}; available balance is total minus held.
	\item Students purchase Marks in approved packages: Starter 50 Marks / Rp 430,000, Learner 120 Marks / Rp 990,000, Explorer 200 Marks / Rp 1,570,000, Pioneer 300 Marks / Rp 2,180,000.
	\item Purchased Marks cannot be converted back to rupiah and do not expire. Unused purchased Marks remain in the student's wallet.
	\item Tutor payout uses the fixed approved payout rate of Rp 7,000 / Mark.
	\item The reference student purchase rate for illustrations is Rp 7,250 / Mark (Pioneer Pack); the working-average rate of approximately Rp 7,800 / Mark is available for Phase 0 illustrations. Displayed IDR prices include the payment gateway processing fee.
...
\section{Group Booking Pricing and Deadline Rules}

Group booking pricing is based on the final confirmed headcount and the tutor's published price for that group size. Phase 0 shall not introduce hidden discount tiers or tier-based pricing.

Tutor cards and tutor profiles show booking prices in Marks before the student starts booking. Class for 1 sessions show the per-student price for size 1. Class for 2 through class for 6 sessions show the starting group price or pricing basis for that size, and the final payable amount is recalculated against confirmed headcount during confirmation or reconfirmation.

\textbf{Standard 90-minute floor pricing}

Each tutor must publish one price for each group size from 1 to 6 students. Published prices are per student, stored in Marks, and must be at or above the Cogito floor price below. There is no ceiling. Tutor price changes apply only to future bookings; confirmed bookings keep their stored price snapshot.

\textit{Online classes at floor price}

\begin{tabular}{|p{2.5cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|}
\hline
\textbf{Students} & \textbf{Floor / Student} & \textbf{Total Pooled} & \textbf{Tutor Share} & \textbf{Cogito Take} \\
\hline
1 & 42 Marks & 42 Marks & 30 Marks & 12 Marks \\
\hline
2 & 35 Marks & 70 Marks & 54 Marks & 16 Marks \\
\hline
3 & 28 Marks & 84 Marks & 64 Marks & 20 Marks \\
\hline
4 & 24 Marks & 96 Marks & 74 Marks & 22 Marks \\
\hline
5 & 21 Marks & 105 Marks & 81 Marks & 24 Marks \\
\hline
6 & 19 Marks & 114 Marks & 88 Marks & 26 Marks \\
\hline
\end{tabular}

\textit{Offline classes at Cogito campus at floor price}

\begin{tabular}{|p{2.5cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|p{2.8cm}|}
\hline
\textbf{Students} & \textbf{Floor / Student} & \textbf{Total Pooled} & \textbf{Tutor Share} & \textbf{Cogito Take} \\
\hline
1 & 50 Marks & 50 Marks & 35 Marks & 15 Marks \\
\hline
2 & 45 Marks & 90 Marks & 70 Marks & 20 Marks \\
\hline
3 & 40 Marks & 120 Marks & 95 Marks & 25 Marks \\
\hline
4 & 35 Marks & 140 Marks & 115 Marks & 25 Marks \\
\hline
5 & 30 Marks & 150 Marks & 120 Marks & 30 Marks \\
\hline
6 & 27 Marks & 162 Marks & 127 Marks & 35 Marks \\
\hline
\end{tabular}

The tables above define Cogito's baseline / minimum price schedule, not a maximum price schedule. Tutors may set prices above the floor for any supported group size. At the baseline price, tutor share and Cogito take follow the corresponding table row. When the tutor-set price is above the baseline, the system calculates the increase at the total pooled level and applies the extra-take rule below.

\textbf{Extra-take rule for above-baseline tutor prices}
\begin{itemize}
    \item Baseline total = baseline floor per student $\times$ final confirmed headcount.
    \item Tutor total = tutor-set per-student price $\times$ final confirmed headcount.
    \item Extra total = tutor total $-$ baseline total.
    \item If extra total is less than 0, the price is invalid and cannot be published.
    \item Cogito extra take = $\lfloor$ extra total $/ 5 \rfloor$ Marks.
    \item Tutor extra share = extra total $-$ Cogito extra take.
    \item Final Cogito take = baseline Cogito take + Cogito extra take.
    \item Final tutor share = baseline tutor share + tutor extra share.
    \item Tutor payout converts final tutor share Marks to IDR at Rp 7,000 / Mark.
\end{itemize}

This means Cogito receives 1 additional Mark for every full 5 Marks of accumulated price increase above the baseline total. Any remainder below the next 5-Mark threshold goes to the tutor. For example, if extra total is 4 Marks, Cogito extra take is 0 and all 4 Marks go to the tutor. If extra total is 5 Marks, Cogito extra take is 1 and 4 Marks go to the tutor. If extra total is 12 Marks, Cogito extra take is 2 and 10 Marks go to the tutor.

\textbf{Pricing examples}

\begin{itemize}
    \item Online class for 1 at floor: student pays 42 Marks, tutor share is 30 Marks, Cogito take is 12 Marks.
    \item Online class for 1 above floor: tutor sets 50 Marks. Extra total = 50 $-$ 42 = 8 Marks. Cogito extra take = $\lfloor 8 / 5 \rfloor$ = 1 Mark. Tutor extra share = 7 Marks. Final tutor share = 37 Marks. Final Cogito take = 13 Marks.
    \item Online class for 3 at floor: each student pays 28 Marks, total pooled is 84 Marks, tutor share is 64 Marks, Cogito take is 20 Marks.
    \item Online class for 3 above floor: tutor sets 32 Marks per student. Tutor total = 96 Marks. Extra total = 96 $-$ 84 = 12 Marks. Cogito extra take = $\lfloor 12 / 5 \rfloor$ = 2 Marks. Tutor extra share = 10 Marks. Final tutor share = 74 Marks. Final Cogito take = 22 Marks.
\end{itemize}
...
\textbf{Rules}
\begin{itemize}
    \item Each group booking stores target group size, minimum confirmed headcount, deadline at, provisional total, and final total.
    ...
    \item Final total = per-participant rate $\times$ confirmed headcount.
    ...
    \item {\sloppy If confirmed headcount is 2 or more but below target, the system enters awaiting\_reconfirmation and sends the final price to all remaining participants.\par}
    \item Every remaining participant, including the proposer if they are part of the group, must reconfirm the repriced total.
    ...
    \item If a confirmed group participant withdraws before H-2, the system releases that participant's held Marks, recalculates the final total for the remaining confirmed headcount, and moves the booking to awaiting\_reconfirmation.
...
\begin{longtable}{|L{1.2cm}|L{3.3cm}|L{3.9cm}|L{6.0cm}|}
	...
	\hline
	TC-05          & Verify tutor prices respect Cogito floors.                      & Tutor and admin can edit tutor pricing.                                    & 1. Try to publish a price below the floor. 2. Publish valid prices for group sizes 1--6. 3. Open the tutor card and profile. \newline Below-floor prices are rejected, valid prices are displayed, and no tier-based pricing is used.                                                                                                                                                                       \\
	\hline
	TC-06          & Verify above-floor tutor pricing split.         & Tutor prices are configured above floor for at least one individual class and one group class.                                        & 1. Set online class for 1 to 50 Marks. 2. Complete the class and inspect the payout split. 3. Set online class for 3 to 32 Marks per student, confirm 3 students, complete the class, and inspect the payout split. \newline For class for 1, final split is tutor 37 Marks and Cogito 13 Marks. For class for 3, final split is tutor 74 Marks and Cogito 22 Marks. In both cases Cogito extra take equals one Mark per full 5 Marks accumulated above baseline.                                                                                                                                                                    \\
	\hline
	TC-07          & Verify tutor listing shows the required booking information.         & Multiple tutors are configured.                                        & 1. Open the tutor list. 2. Review each tutor card. \newline Each card shows summary profile, expertise, tutor-set Marks prices, and availability.                                                                                                                                                                    \\
	\hline
...
\begin{itemize}
    ...
    \item Implement the booking, Marks ledger, and override domains test-first. The highest-risk rules are 12-hour response windows, above-floor tutor pricing split, no Marks-to-rupiah conversion, admin-verified tutor no-show return, and payment-error reconciliation.
...
\begin{itemize}
	...
	\item \textbf{Floor Price}: minimum per-student price a tutor may set for a group size.
	\item \textbf{Baseline Total}: floor price per student multiplied by final confirmed headcount.
	\item \textbf{Extra Total}: tutor-set total price minus baseline total.
	\item \textbf{Cogito Take}: baseline number of Marks Cogito deducts from total class revenue at session completion, plus 1 extra Mark for every full 5 Marks accumulated above the baseline total.
	\item \textbf{Tutor Net}: baseline tutor share plus remaining above-baseline Marks after Cogito extra take, converted to IDR at the tutor payout rate.
	\item \textbf{Exchange Margin}: spread between student purchase rate and tutor payout rate on Marks flowing through the platform.
