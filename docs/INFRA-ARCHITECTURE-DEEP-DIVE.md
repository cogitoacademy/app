...
> from memory. R2 browser uploads were revalidated and corrected on 2026-09-01.
> Companion to `docs/DEPLOYMENT.md` and
...
| **Provider API keys**      | Each vendor's own credential (Resend, Midtrans, R2, Sanity)                                         | API server env                  | Email, payments, storage, content |
...
1. **The Age private key must never touch CI.** If it lived in GitHub
   ...
   could decrypt the entire vault — payment-provider keys, Google credentials, DB
...
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Midtrans**           | ✅ Snap integration (`POST /snap/v1/transactions`, hosted `redirect_url`, body `signature_key`, status mapping, refund port, webhook lifecycle idempotency). Sandbox keys work.                                                                                                                                                     | Env schema **fails boot** if `PAYMENT_PROVIDER=midtrans` lacks Server Key, Client Key, merchant id, or `MIDTRANS_MODE`; production/staging Sandbox also requires `PAYMENT_TEST_ALLOWED_EMAILS`. |
...
| **R2**                 | ✅ AWS SDK presigned PUT uploads with exact content type/length, browser bucket CORS, all-or-nothing env guard, and `R2_PUBLIC_URL` required in prod. **Two buckets (2026-08-28):** public `cogito-bucket` (uploads, `r2bucket.cogitoacademy.id`) vs private `cogito-backups` (`R2_BACKUP_BUCKET`, dumps/snapshots — never public). | **Fails boot** in prod without all four `R2_*` + `R2_PUBLIC_URL` (P4.3).                                                                                                                        |
...
| Item                                                                                            | State                                                                                                              | Blocker?                                                              |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------- |
| R2 bucket + token                                                                               | **DONE 2026-08-31** — `cogito-bucket` (public uploads) + `cogito-backups` (private) in Terraform state, zero drift | —                                                                     |
| SOPS vault                                                                                      | **DONE 2026-08-28** — filled (44 keys) + encrypted + committed; Coolify/R2 tokens rotated 2026-08-31               | —                                                                     |
| Google Meet refresh token                                                                       | Missing                                                                                                            | Not a blocker — set `GOOGLE_MEET_ENABLED=false`, manual links work    |
| Midtrans                                                                                        | Sandbox keys                                                                                                       | Not a blocker — sandbox E2E first, then one real transaction          |
...
| `coolify-resources.yml` (Ansible → Coolify API, incl. the Traefik route fixing the webhook 401) | **DONE 2026-08-31** — applied via `infra/apply.sh` (47 env vars, webhook route live)                               | —                                                                     |
| Terraform apply, Tailscale join, Ansible applies                                                | **DONE 2026-08-31** — 7 resources in state, VPS joined + hardened (ACL pasted 2026-09-01)                          | Remaining drills in DEPLOYMENT-PLAN Phase 5                           |
...
| `PAYMENT_PROVIDER` / `PAYMENT_TEST_ALLOWED_EMAILS`             | Coolify/SOPS vault                                | ✅ (`midtrans` + UAT list in Sandbox)      |
| `MIDTRANS_SERVER_KEY` / `MIDTRANS_CLIENT_KEY`                  | Midtrans dashboard → Access Keys                  | ✅ when `PAYMENT_PROVIDER=midtrans`        |
| `MIDTRANS_MERCHANT_ID` / `MIDTRANS_MODE`                       | Midtrans dashboard + deployment assertion         | ✅ when `PAYMENT_PROVIDER=midtrans`        |
| `MIDTRANS_WEBHOOK_SIGNATURE_KEY`                               | Optional dedicated verification secret            | optional; Server Key is fallback           |
