...
Backend is ready for user role management, tutor invite/review, achievement moderation, the full booking operations console (queue/override preview/refund), room list/create/assign/relocate, wallet/ledger lookup, tutor payouts, refund corrections, the active economy schedule, and manual meeting-link fallback for eligible online bookings. The /admin-economy screen lets admins edit the four Cogito take fields in Rp 5,000 increments with optimistic versioning; updates are audit-logged and apply only to future/new repricing snapshots. Achievement moderation remains the safest next Admin UI quick win.
...
3. Close the production deployment-readiness items in `docs/plans/active/DEPLOYMENT-FINAL.md` before enabling real payments and scheduled jobs.
