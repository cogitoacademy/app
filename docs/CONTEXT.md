...
Booking detail uses a task-detail layout shared by student and tutor views: a compact identity-and-status header, a primary content flow for overview, series sessions, notes/reports, and activity, plus a sticky metadata rail that places available Booking actions above Marks. The overview keeps participant names and profile images visible without a separate low-priority rail card. All existing lifecycle actions and data remain available without changing the booking API.
