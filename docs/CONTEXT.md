...
- **CD**: GitHub Actions builds server/web images and pushes them to GHCR on a hosted runner. A dependent deploy job runs on the production VPS runner (`[self-hosted, linux, x64, production]`), where `scripts/resolve-private-db-url.sh` safely resolves the Coolify-private PostgreSQL container without publishing port 5432, then `scripts/migrate-and-deploy.sh` performs pg_dump → private R2 snapshot → `bun run db:migrate` → Coolify webhook with `force=true` → sha-verified health poll. Forced deployment makes Coolify pull the current registry image instead of restarting a cached `latest` image. The `DATABASE_URL` variable is allowlisted for Turbo's strict `db:migrate` task so `drizzle-kit` receives the resolved production URL. The jobs use non-cancelling `production-deploy` concurrency so two production migrations cannot overlap. The server image embeds `GIT_SHA`; a green deploy proves `/health.version` matches the merged commit. Failed verification uses Coolify's native rollback endpoint with the UUID from the webhook, requiring deploy access without application configuration writes.
