...
Tutor onboarding renders the competition category cards as two independent vertical stacks at the `md` breakpoint and above, placing alternating categories in each stack so a tall category cannot create height-driven empty space beside shorter cards. Below `md`, the stacks collapse into one visible column. This is frontend-only presentation behavior; taxonomy, selection limits, RPC inputs/outputs, and persistence are unchanged.
