...
| H2  | HIGH     | Tutor-lateness handling flawed both ways: pre-marking attendance dodges detection; taught-but-unmarked sessions auto-cancel + release holds. **FIXED** (REVIEW-FIXES-4 P2.5): `markTutorAttendance` is now allowed only within `[scheduledStartAt ± 15 min]` (`BookingNotEditableError` otherwise, so pre-marking is impossible); `checkTutorLateness` no longer auto-cancels or releases holds — it keeps the session SCHEDULED with holds intact, merges `overrideMeta.category = "tutor_lateness_pending"` (surfacing the booking in the `adminBooking.listBookings({ category })` queue), writes a `tutor_lateness_pending_review` audit record, and notifies the proposer + tutor. NO_SHOW for tutor absence is now an admin decision | `booking.service.ts` (markTutorAttendance, checkTutorLateness), `booking.repo.ts` |
