...
Online meeting status stays compact as an accessible Selia `IconInfoSquareRounded` trigger for both unavailable and available links. The popover contains the pending/failed explanation and retry/admin setup badge when needed; when `meetingUrl` exists, it contains the meeting-room action instead of a `Ready` badge or standalone CTA. Missing offline-room details and tutor completion timing use the same trigger beside the related status or action, opening the explanation on hover, focus, click, or touch. This is a presentation-only refinement with no API or meeting-state contract change.
