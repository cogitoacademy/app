...
- **CD**: GitHub Actions builds server/web images and pushes them to GHCR on a hosted runner. A dependent deploy job runs on the production VPS runner (`[self-hosted, linux, x64, production]`), where `scripts/resolve-private-db-url.sh` safely resolves the Coolify-private PostgreSQL container without publishing port 5432, then `scripts/migrate-and-deploy.sh` performs pg_dump → private R2 snapshot → `bun run db:migrate` → pin Coolify to immutable `v<GIT_SHA>` → deploy webhook → sha-verified health poll. Explicitly advancing the tag prevents a prior auto-rollback's `v<PREV_GIT_SHA>` pin from trapping all later webhooks on the old image. The `DATABASE_URL` variable is allowlisted for Turbo's strict `db:migrate` task so `drizzle-kit` receives the resolved production URL. The jobs use non-cancelling `production-deploy` concurrency so two production migrations cannot overlap. The server image embeds `GIT_SHA`; a green deploy proves `/health.version` matches the merged commit. Failed verification attempts a best-effort API rollback using the safely initialized canonical Coolify API URL; `COOLIFY_APP_UUID` is an optional repository secret that bypasses domain lookup.
