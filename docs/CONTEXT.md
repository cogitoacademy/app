...
- **Observability No-Data wave (2026-09-06, branch `fix/obs-grafana-no-data`):** live audit found three healthy-but-empty panels — 5xx error-rate exprs (empty numerator when zero 5xx) made zero-safe with `or vector(0)` (App RED, Delivery, ApiErrors alert), `breaker_state` panels made healthy-safe (`or on() vector(0)` / `max(...) or vector(0)`; healthy exposition omits the section when no `cogito:cb:*` keys exist). Container-memory panel root cause: cAdvisor `v0.52.1` predates containerd-snapshotter support (upstream #3709, fixed in `v0.54.0`) and Docker 29.7.2 uses the containerd image store, so cAdvisor drops every Docker container (only `id/instance/job` labels, no `name`/`container_label_*`); fix is image bump to `ghcr.io/google/cadvisor:v0.60.5` in `observability.yml` — **pending operator apply + `cogito-alloy` redeploy** (see `docs/plans/active/OBS-GRAFANA-NODATA.md`). Alert-rule changes take effect via the apply run or the Infra Apply pipeline (Play 2 syncs the tree and auto-restarts Grafana; file-provisioned rules load at startup only).
