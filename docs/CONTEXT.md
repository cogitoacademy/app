...
- **Observability stack PLG (declared 2026-09-05, NOT yet applied — operator apply pending):** `infra/ansible/observability.yml` (control-node driven, modeled on `uptime-kuma.yml`, idempotent) declares Coolify services `cogito-loki` (volume `loki-data`), `cogito-prometheus` (`prometheus-data`), `cogito-grafana` (`grafana-data`, tailnet-only, no public domain — drift-checked), and the `cogito-alloy` bundle (Alloy via `loki.source.docker_logs` Docker-API discovery + node_exporter + cAdvisor). Provisioned files: `infra/prometheus/prometheus.yml` (api `/metrics` Bearer-from-vault-token-file, 15s), `infra/loki/loki-config.yml` (30d), `infra/alloy/config.alloy`, Grafana datasources + 4 dashboards (App RED, Logs & Traces with traceId/userId search, Infra with 85%/92% lines, Delivery). Retention `LOKI_RETENTION_DAYS=30` / `PROM_RETENTION_DAYS=15` (lean fallback 30s/7d documented in RUNBOOK); 2G swap + memory limits (API 512M, Loki 300M, Prometheus 256M, Grafana 256M) documented as operator steps; `ops.sh trace <traceId>` prints the tailnet Grafana Explore URL. Logs carry `userId`, never email.
