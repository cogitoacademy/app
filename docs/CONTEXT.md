...
- The `/login` email forms validate each touched field on change and blur, keep the client rules aligned with the server password policy, show inline field errors plus an invalid-submit warning, and normalize name/email whitespace before calling Better Auth. This is presentation and client-validation behavior only; it does not change auth endpoints or persistence.
