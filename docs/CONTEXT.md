...
- The authenticated calendar keeps the academy's full read-only interaction model: month view with multi-day event spans and overflow popup, 30-day agenda view, keyboard shortcuts (`M`/`A`), period navigation, and a responsive event-details modal. Its colors, controls, and icons use the app's Selia design system; the academy's bilingual copy is not carried into the English-only app.
...
### 2026-08-25 audit findings (tracked in `docs/plans/active/BACKEND-PROD-FINALIZATION.md`)

Status: all fixed by the backend finalization branch (planned in `docs/plans/active/BACKEND-PROD-FINALIZATION.md`, PR pending). Findings come from the two-worker audit (worker A: business-logic/data layer; worker B: server/ops/CI) plus lead re-verification.
...
| S4 | MEDIUM | Auth rate limiter path prefixes don't match real better-auth endpoints (`/request-password-reset` vs `/request-password-reset/`) | prefix list corrected to actual endpoint paths |
