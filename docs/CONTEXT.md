...
- **Pending (foundation hardening):** password policy — **fully implemented (C6 closed by REVIEW-FIXES-3 P6)** — min 8 via `minPasswordLength` + upper/lower/digit via `assertPasswordPolicy` enforced in the server auth route at sign-up (`apps/server/src/routes.ts`), mirrored in the sign-up form (single consistent statement — 2026-08-17). Conditional Google OAuth — implemented (gated on env vars). Session expiry is set (7 days, `expiresIn`). **Email verification (G2) deferred** (additive; depends on Resend wiring + frontend route).
...
| Plan                                                              | Branch                             | Status                                                                                                         |
| ----------------------------------------------------------------- | ---------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `docs/plans/active/REVIEW-FIXES-4.md`                             | `fix/review-fixes-4` (future PRs)  | Active — wave-4 audit fixes (docs/sdd reconciliation, money bugs C1–M9, Xendit rewrite, fail-loud 3P guards)   |
| `docs/plans/active/PRD-GAPS-PHASE3.md`                            | main (merged)                      | Active — all U-items closed except U9 partial (U9 → REVIEW-FIXES-4 P2.8)                                       |
| `docs/plans/active/FRONTEND-GAPS-SPEC.md`                         | `f/frontend-prd-gaps` (merged #55) | Active — F-items closed by wave-3 P6; F1/F9/F12 partial                                                        |
...
| `docs/plans/completed/REVIEW-FIXES-3.md`                          | main (merged)                      | Merged to main (#59–#65) — all wave-3 PRs landed; only G2 (email verification) still deferred                  |
...
10. Review Fixes 4 (REVIEW-FIXES-4.md — docs/sdd reconciliation, money bugs C1–M9, Xendit rewrite, fail-loud guards) → next to execute
11. Frontend Gaps (FRONTEND-GAPS-SPEC — F-items closed by wave-3 P6; F1/F9/F12 partial) → after / parallel with #10
...
Production Readiness (#18) and Infrastructure (#19) merged to main. Deferred ops code gaps (1.1–1.8) are merged; Redis session caching remains deferred. PRD gaps backend (G1–G20) landed on main, and **BACKEND-HARDENING-PHASE2 (PRs 1–6) merged to main via #46** — security hardening, group-booking money correctness, late-cancel penalty, email outbox, R2 uploads, group-series, deadline repricing, payment notifications, meeting event lifecycle, SLA escalation. **BACKEND-REVIEW-HARDENING merged to main via #48** — the 2026-08-15 review fixes (money correctness, security, reliability, Redis mandatory). **REVIEW-FIXES-2 merged via #50–#57** (wave-2 findings), **REVIEW-FIXES-3 merged via #59–#65** (wave-3 findings). Next: **REVIEW-FIXES-4 (wave-4: docs/sdd reconciliation, money-correctness bugs C1–C3/H1–H6/M1–M9/L1–L5, Xendit provider rewrite for the 2024-11-11 API, fail-loud Resend/Google Meet/R2 guards)**, then remaining frontend-gap work and production ops.
...
The admin override queue, wallet/ledger view, override preview, room assignment → scheduled transition + notifications, and room availability/approval backend (G8–G10, G13–G14) have landed; the corresponding admin UI remains frontend work (F1/F2/F11/F12). Backend U-item sub-gaps are tracked in `docs/plans/active/PRD-GAPS-PHASE3.md` (all closed except U9 partial → REVIEW-FIXES-4 P2.8).
...
### 2026-08-17 wave-4 findings (tracked in `docs/plans/active/REVIEW-FIXES-4.md`)

| ID  | Severity | Finding                                                                                                                                                                                                                                                                                                                                                                                                                       | Location                                                    |
| --- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| C1  | CRITICAL | Group `markParticipantNoShow` transitions the whole booking to NO_SHOW and strands other participants' holds (PRD: one no-show forfeits only their own Marks)                                                                                                                                                                                                                                                                 | `booking.service.ts` (markParticipantNoShow)                |
| C2  | CRITICAL | Student bypasses the H-2 late-cancel penalty via `proposeReschedule` (no guard that the **current** session is still > H-2 out)                                                                                                                                                                                                                                                                                               | `booking.service.ts` (proposeReschedule)                    |
| C3  | CRITICAL | `completeSession` has no "session started" guard (tutor can deduct held Marks before the session); `completeSeriesSession` has one                                                                                                                                                                                                                                                                                            | `booking.service.ts` (completeSession)                      |
| H1  | HIGH     | Accepted reschedule keeps the deadline at proposal `now+24h` — a reschedule for a session 2 weeks out is auto-cancelled ~24h later and holds released                                                                                                                                                                                                                                                                         | `booking.service.ts` (propose/accept/expire)                |
| H2  | HIGH     | Tutor-lateness handling flawed both ways: pre-marking attendance dodges detection; taught-but-unmarked sessions auto-cancel + release holds                                                                                                                                                                                                                                                                                   | `booking.repo.ts`, `booking.service.ts`                     |
| H3  | HIGH     | `relocateRoom` doesn't transition `AWAITING_ADMIN_ROOM_APPROVAL` → SCHEDULED (unlike `assignRoom`), so relocated bookings get expired by the deadline job                                                                                                                                                                                                                                                                     | `room.service.ts` (relocateRoom)                            |
| H4  | HIGH     | REFUNDED webhook auto-reversal throws when Marks already spent → webhook 500/retry loop instead of admin reconciliation                                                                                                                                                                                                                                                                                                       | `payment.service.ts`, `wallet.repo.ts`                      |
| H5  | HIGH     | Support SLA is flat 12h; no business-hours/WIB computation or WhatsApp escalation; no auto-ack on ticket creation (PRD OQ-04)                                                                                                                                                                                                                                                                                                 | `constants.ts`, `support.service.ts`                        |
| H6  | HIGH     | `applyOverride` to terminal states never cancels the Google Meet event (unlike every other terminal path)                                                                                                                                                                                                                                                                                                                     | `admin-booking.service.ts` (applyOverride)                  |
| M1  | MED      | `applyOverride` with `marksAction` but empty `affectedParticipants` silently no-ops the money action → stranded holds                                                                                                                                                                                                                                                                                                         | `admin-booking.service.ts`                                  |
| M2  | MED      | `expireBookings` SCHEDULED→NO_SHOW **releases** holds instead of forfeiting (PRD: no-show → deduct)                                                                                                                                                                                                                                                                                                                           | `booking.service.ts` (expireBookings)                       |
| M3  | MED      | Proposer `cancel()` of a confirmed group **series** bypasses the no-opt-out guard and releases every participant's holds                                                                                                                                                                                                                                                                                                      | `booking.service.ts` (cancel vs withdraw)                   |
| M4  | MED      | `releaseExpiredHolds` releases holds without a state transition → later tutor accept/complete deducts from a zero hold (`InsufficientBalanceError` → delivered-but-unpaid session)                                                                                                                                                                                                                                            | `booking.service.ts` (releaseExpiredHolds)                  |
| M5  | MED      | Reconfirmation-decline / withdraw-pre-H2 don't refresh `deadlineAt` to `now+12h` → sub-12h reconfirmation windows                                                                                                                                                                                                                                                                                                             | `booking.service.ts`                                        |
| M6  | MED      | `cancelRoomBooking` (admin) only cancels the roomBooking row; the booking stays `AWAITING_ADMIN_ROOM_APPROVAL` until its deadline expires it (PRD FR-22 expects cancel + hold release + audit)                                                                                                                                                                                                                                | `room.service.ts` (cancelRoomBooking)                       |
| M7  | MED      | Withdraw from `AWAITING_ADMIN_ROOM_APPROVAL` regresses to reconfirmation but leaves the `requested` roomBooking live for admin assign                                                                                                                                                                                                                                                                                         | `booking.service.ts` (withdraw)                             |
| M8  | MED      | Pre-H2 withdraw group reprice throws `InsufficientMarksError` and rolls back the withdrawal (PRD TC-19: fall through to expiry)                                                                                                                                                                                                                                                                                               | `booking.service.ts` (withdraw/repriceGroupForHeadcount)    |
| M9  | MED      | Knowledge Bank eligibility is `protectedProcedure`, not student-only (PRD FR-12); tutors/admins with ≥35 Marks pass the gate                                                                                                                                                                                                                                                                                                  | `wallet.router.ts`                                          |
| L1  | LOW      | `completeSeriesSession` can deduct more than the remaining hold after an admin `cancelSeriesSession(..., release)` → `InsufficientBalanceError`; guard with `Math.min`                                                                                                                                                                                                                                                        | `admin-booking.service.ts`, `booking.service.ts`            |
| L2  | LOW      | `meetingEvent` row written on the global `db`, not the booking tx → tx rollback after Google event creation leaves an orphan event + row; tutor re-accept duplicates the event                                                                                                                                                                                                                                                | `google-meeting.provider.ts`, `booking.service.ts`          |
| L3  | LOW      | "Meeting link ready" notification sent even with no URL (fallback provider) — should say "link pending"                                                                                                                                                                                                                                                                                                                       | `booking.service.ts`                                        |
...
| X1  | —        | **Xendit NOT production-compatible** — provider uses the legacy pre-2024 v3 shape; current API (`api-version: 2024-11-11`) needs `type:"PAY"`, `request_amount`, `channel_code`, `channel_properties`, `customer`, top-level response with `actions[].value`, statuses `SUCCEEDED/REQUIRES_ACTION/AUTHORIZED/CANCELED`, webhook `data.payment_id` (no `event_id` → idempotency key collision), and a provider **refund** port | `xendit-payment.provider.ts`, `payment.service.ts`          |
| X2  | —        | Resend: correct provider; missing `NODE_ENV=production` requirement on `RESEND_API_KEY` → silent stub suppresses all critical emails with no alert                                                                                                                                                                                                                                                                            | `packages/env/src/server.ts`                                |
| X3  | —        | Google Meet: both auth paths correct; missing `GOOGLE_IMPERSONATED_USER` guard for SA mode (events land on the SA's own calendar), no boot-time probe, `.env.prod.example` broken                                                                                                                                                                                                                                             | `google-meeting.provider.ts`, `infra/.env.prod.example`     |
| X4  | —        | R2: presigned POST correct; missing prod guard (no-R2 prod writes to container-local `UPLOAD_DIR`, lost on redeploy) and `R2_PUBLIC_URL` requirement (objects unreachable when R2 set but URL unset)                                                                                                                                                                                                                          | `storage.ts`, `packages/env/src/server.ts`                  |
