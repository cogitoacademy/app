...
- **CD**: GitHub Actions builds server/web images → pushes to GHCR → runs the production pipeline through `scripts/migrate-and-deploy.sh` (pg_dump snapshot → gzip → R2 `pre-migrate-<sha>.sql.gz` → `bun run db:migrate` → Coolify webhook → poll `https://api.cogitoacademy.id/health` until `version == GIT_SHA`, bounded 20×15s, with a rollback hint on failure). The server image is built with `--build-arg GIT_SHA=${{ github.sha }}` (Dockerfile `ARG`/`ENV GIT_SHA`) and `GET /health` returns it as `version` (`"dev"` when unset), so the CD poll verifies the deployed sha, not just "some container is up". `cd-prod.yml` guards the Coolify webhook secrets before any curl (unset secret → clear message + `exit 1`); the manual GHCR/Coolify fallback is documented in `docs/DEPLOYMENT.md`
