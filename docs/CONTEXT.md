...
- **CD**: GitHub Actions builds server/web images and pushes them to GHCR on a hosted runner. A dependent deploy job runs on the production VPS runner (`[self-hosted, linux, x64, production]`), logs into GHCR with package-read permission, and pulls immutable `server:v<GIT_SHA>` into the Docker image store shared with Coolify. It then resolves the private database, runs pg_dump → private R2 snapshot → `bun run db:migrate`, selects the now-local immutable image through Coolify's native rollback/deployment endpoint, and performs the sha-verified health poll. This uses Coolify deploy access without PATCHing application configuration, even when the resource's configured tag points elsewhere. A green deploy proves `/health.version` matches the merged commit; failed verification selects the locally retained `v<PREV_GIT_SHA>` through the same endpoint.
