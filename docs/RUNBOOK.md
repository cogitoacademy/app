...
- Complete-session feedback: as tutor, open a scheduled booking past end time, click **Complete session**, fill Session discussion / Identified strengths / Points of improvement (Enter adds bullet), confirm disabled until all 3 have ≥1 bullet, submit and verify success toast + booking completed. As student/admin, open the completed booking and verify the Session feedback card lists all 3 bullet sections (per-session rows for series).
