...
- Room conflict-check/write paths take a transaction-scoped advisory lock keyed by `roomId`. Migration `0038_room_booking_overlap_guard.sql` adds the database backstop: a partial GiST exclusion constraint rejects overlapping `[startAt,endAt)` ranges for `confirmed` rows in the same room while allowing adjacent sessions.
