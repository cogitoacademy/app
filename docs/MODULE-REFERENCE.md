...
- `updateEvent(bookingId, changes)` — Best-effort updates provider-side start/end and/or physical location while preserving the rest of the event resource. Updates use `sendUpdates=none` and `conferenceDataVersion=1` so the existing online Meet conference survives the full-resource update without Google emailing attendees (OQ-05, #46). Cogito owns booking notifications; Calendar attendee emails stay suppressed on create/update/delete to avoid duplicate mail and DSN bounces to undeliverable seed addresses.
