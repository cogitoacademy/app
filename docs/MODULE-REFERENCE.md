...
- `reviewTutorProfile(profileId, status, adminNote?)` — Approve/reject tutor profile. **F25 state machine (`validateReviewAction`):** each action is only allowed from specific onboarding statuses — `request_changes`/`approve_unpublished`/`publish` from `pending_review`/`changes_requested` (publish also from `approved_unpublished`); `unpublish`/`suspend`/`approve_edits`/`request_edit_changes` only from `published`. Anything else throws `InvalidInviteActionError` (e.g. `publish` from `suspended`, `request_changes` from `published`). **N2 (suspended restore, admin-only):** `approve_unpublished` and `request_changes` are also allowed from `suspended`, so an admin can reinstate a suspended profile (PRD FR-24) — `approve_unpublished` restores it to the pre-publication review state and `request_changes` sends it back to the tutor for re-editing; `publish` stays blocked from `suspended` (restore is never a blind re-publish) and the tutor cannot self-restore via `submitForReview` (which requires `draft`/`changes_requested`)
