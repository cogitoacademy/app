...
| Observability  | `ansible-playbook ... drift-check.yml ...` exits 0 (covers PLG/studio: existence, `urls == []`, image pins); Prometheus targets UP (tunnel 9090); LogQL `{service="cogito-api"} | = "traceId"`returns rows;`./infra/ops.sh trace <traceId>` prints the tailnet Explore URL |
