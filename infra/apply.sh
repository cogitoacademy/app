...
phase_observability() {
  ...
  say "  config changes self-restart cogito-prometheus + cogito-alloy + cogito-grafana via the playbook handler (change-triggered only); first-ever apply: confirm the restarts took (Prometheus targets UP + Grafana login over the tailnet)."
