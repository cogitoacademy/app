...
function CalendarSelect(props: React.ComponentProps<'select'>) {
  ...
  return (
    <div className="relative inline-flex">
      ...
      <IconChevronDown
        className="absolute right-1.5 top-1/2 -translate-y-1/2 size-3.5 text-muted pointer-events-none"
      />
    </div>
  );
}
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        button_previous: 'col-start-1 size-7 grid place-items-center rounded hover:bg-accent transition-colors',
        button_next: 'col-start-3 size-7 grid place-items-center rounded hover:bg-accent transition-colors',
        chevron: 'size-4 text-foreground',
        month_grid: 'col-span-3 w-full border-collapse',
        weekdays: 'flex',
        weekday: 'size-8 flex-1 text-xs text-muted flex items-center justify-center font-normal',
        weeks: 'flex flex-col gap-1 mt-2',
        week: 'flex w-full gap-1',
        day: 'size-8 flex-1 text-center text-sm text-foreground p-0',
        day_button: 'size-8 rounded grid place-items-center cursor-pointer transition-colors hover:bg-accent w-full',
