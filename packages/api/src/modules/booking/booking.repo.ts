...
export function createBookingRepo(db: DbType) {
  ...
  async function listSessionsBySeriesId(conn: DbOrTx, seriesBookingId: string) {
    return conn
      .select()
      .from(bookingSession)
  ...
  async function findBookingsExpiringByDeadline(
    conn: DbOrTx,
    states: string[],
  ) {
    return conn
  ...
  async function findBookingType(
    conn: DbOrTx,
    bookingId: string,
  ): Promise<string | null> {
    const [b] = await conn
  ...
  async function updateBookingVersioned(
    ...
    const result = await conn
      ...
      .where(
        and(eq(booking.id, bookingId), eq(booking.version, expectedVersion)),
      )
      .returning();
